...
const Model = require('../../models/models'); // Class-based MySQL model
...
// GET: Edit Model (Load existing data)
    ...
    try {
        ...
        for (let i = 1; i <= exteriorCount; i++) {
            if (req.body[`exteriorColorName${i}`]) {
                const colorImage = req.files.find(f => f.fieldname === `exteriorColorImage${i}`);
                ...
                for (let j = 1; j <= 10; j++) {
                    const img = req.files.find(f => f.fieldname === `exteriorAdditionalColorImage${i}_${j}`);
                    if (img) {
                        await Model.createExteriorColorImage(colorId, img.path.replace('public', '').replace(/\\/g, '/'));
                    }
                }
            }
        }
        ...
        for (let i = 1; i <= interiorCount; i++) {
            if (req.body[`interiorColorName${i}`]) {
                const colorImage = req.files.find(f => f.fieldname === `interiorColorImage${i}`);
                const colorId = await Model.createInteriorColor(modelId, {
                    name: req.body[`interiorColorName${i}`]
                }, colorImage ? colorImage.path.replace('public', '').replace(/\\/g, '/') : null);

                for (let j = 1; j <= 10; j++) {
                    const img = req.files.find(f => f.fieldname === `interiorAdditionalColorImage${i}_${j}`);
                    if (img) {
                        await Model.createInteriorColorImage(colorId, img.path.replace('public', '').replace(/\\/g, '/'));
                    }
                }
            }
        }
        ...
        for (let i = 1; i <= aboutCount; i++) {
            ...
            if (type === 'article' && req.body[`aboutContent${i}`]) {
                await Model.createAboutContent(modelId, {
                    type: 'article',
                    value: req.body[`aboutContent${i}`]
                }, i);
            } else if (type === 'photo') {
                const photoFile = req.files.find(f => f.fieldname === `aboutPhoto${i}`);
                if (photoFile) {
                    await Model.createAboutContent(modelId, {
                        type: 'photo',
                        image_path: photoFile.path.replace('public', '').replace(/\\/g, '/'),
        ...
        for (let s = 1; s <= specCount; s++) {
            ...
            for (let l = 1; l <= listCount; l++) {
                ...
                const listId = await Model.createSpecificationList(specId, {
                    title: req.body[`specListTitle${s}_${l}`]
                });
                ...
                for (let c = 1; c <= contentCount; c++) {
                    ...
                    if (cType === 'article' && req.body[`specContent${s}_${l}_${c}`]) {
                        await Model.createSpecContent(listId, {
                            type: 'article',
                            value: req.body[`specContent${s}_${l}_${c}`]
                        });
                    } else if (cType === 'photo') {
                        const photo = req.files.find(f => f.fieldname === `specPhoto${s}_${l}_${c}`);
                        if (photo) {
                            await Model.createSpecContent(listId, {
                                type: 'photo',
...
const updateAdminModel = async (req, res) => {
    ...
    try {
        ...
        await Model.deleteModel(modelId); // This deletes related tables
        await db.execute(`DELETE FROM models WHERE id = ?`, [modelId]); // Delete main

        // Re-insert with new data (reuse create logic)
        return postAdminModel(req, res);
    } catch (err) {
...
// GET: Filtered Models (for dropdown)
