...
const fs = require('fs');
const rootDir = require('../utils/pathutil'); // For absolute path
...
class Model {
    ...
    static async createModel(modelData, imagePath, authorId) {
        ...
        // Normalize image path for database
    ...
    // Get model details (all related data)
    static async getModelDetails(modelId) {
        if (!modelId || isNaN(modelId)) throw new Error('Model ID is required and must be a number');
        
        try {
            const [exteriorColors] = await db.execute(
                `SELECT ec.*, eci.image_path AS additional_image 
            ...
            const [interiorColors] = await db.execute(
                `SELECT ic.*, ici.image_path AS additional_image 
            ...
            const [sites] = await db.execute(`SELECT * FROM available_sites WHERE model_id = ?`, [modelId]);

            const [specifications] = await db.execute(`SELECT * FROM specifications WHERE model_id = ?`, [modelId]);
            const [specLists] = await db.execute(
                `SELECT sl.*, s.title AS spec_title 
                 FROM specification_lists sl
                 JOIN specifications s ON sl.specification_id = s.id
                 WHERE s.model_id = ?`,
                [modelId]
            );
            const [specContents] = await db.execute(
                `SELECT sc.*, sl.title AS list_title 
                 FROM spec_contents sc
    ...
    static async updateModel(id, modelData, imagePath, authorId) {
        ...
        try {
            ...
            if (normalizedImagePath && oldImage) {
            ...
            if (result.affectedRows === 0) throw new Error('Model not found for update');
        } catch (err) {
            console.error('Error in updateModel:', { message: err.message, id });
    ...
    static async deleteModel(id) {
        ...
        try {
            ...
            const tablesWithImages = [
                ...
                'spec_contents', 'about_contents'
            ];

            for (const table of tablesWithImages) {
                const imageField = table.includes('color_images') ? 'image_path' : 
                                 table.includes('contents') ? 'image_path' : 'color_image';
                const idField = table.includes('exterior_color') ? 'exterior_color_id' :
                               table.includes('interior_color') ? 'interior_color_id' :
                               table.includes('spec') ? 'list_id' : 'model_id';

                let query = `SELECT ${imageField} FROM ${table} WHERE `;
                if (table.includes('images')) {
                    query += ` ${idField} IN (SELECT id FROM ${table.replace('_images', 's')} WHERE model_id = ?)`;
                } else {
                    query += ` model_id = ?`;
                }

                const [images] = await db.execute(query, [id]);
                for (const img of images) {
                    if (img[imageField]) {
                        const filePath = path.join(rootDir, 'public', img[imageField]);
            ...
            await db.execute(`DELETE FROM exterior_color_images WHERE exterior_color_id IN (SELECT id FROM exterior_colors WHERE model_id = ?)`, [id]);
            await db.execute(`DELETE FROM interior_color_images WHERE interior_color_id IN (SELECT id FROM interior_colors WHERE model_id = ?)`, [id]);
            await db.execute(`DELETE FROM spec_contents WHERE list_id IN (SELECT id FROM specification_lists WHERE specification_id IN (SELECT id FROM specifications WHERE model_id = ?))`, [id]);
            await db.execute(`DELETE FROM specification_lists WHERE specification_id IN (SELECT id FROM specifications WHERE model_id = ?)`, [id]);
