...
const VechicleType = require('../../models/vechicletype');
    ...
    try {
        ...
        for (const color of exteriorColors) {
            ...
            for (const img of color.additionalImages) {
                ...
                if (imgPath) {
                    await Model.createExteriorColorImage(exteriorColorId, imgPath);
                }
            }
        }

        // Handle interior colors
        ...
        for (const color of interiorColors) {
            ...
            for (const img of color.additionalImages) {
                ...
                if (imgPath) {
                    await Model.createInteriorColorImage(interiorColorId, imgPath);
                }
            }
        }
        ...
        Object.keys(req.body).forEach(key => {
            if (key.startsWith('variantName')) {
                ...
                if (name && price) {
                    variants.push({ name, price: parseFloat(price) });
                }
            }
        });
        for (const variant of variants) {
            await Model.createVariant(modelId, variant);
        }

        // Handle available sites
        const sites = [];
        Object.keys(req.body).forEach(key => {
            if (key.startsWith('siteName')) {
                ...
                if (name && link) { // Require link for sites
                    sites.push({ name, link });
                }
            }
        });
        for (const site of sites) {
            await Model.createAvailableSite(modelId, site);
        }
                        ...
                        if (listKey.startsWith(`specListTitle${index}_`)) {
                            ...
                            if (listTitle) {
                                ...
                                Object.keys(req.body).forEach(contentKey => {
                                    if (contentKey.startsWith(`specContent${index}_${listIndex}_`)) {
                                        ...
                                        if (contentType) {
                                            ...
                                            if (content.value || content.image_path) {
                                ...
                                if (list.contents.length > 0) {
                                    spec.lists.push(list);
                                }
                            }
                        }
                    });
                    if (spec.lists.length > 0) {
        ...
        for (const spec of specifications) {
            ...
            for (const list of spec.lists) {
                ...
                for (const content of list.contents) {
                    await Model.createSpecContent(listId, content);
                }
            }
        }
        ...
        Object.keys(req.body).forEach(key => {
            if (key.startsWith('aboutContent')) {
                ...
                if (contentType) {
                    ...
                    if (content.value || content.image_path) {
                        aboutContents.push(content);
                    }
                }
            }
        });
        for (let i = 0; i < aboutContents.length; i++) {
            await Model.createAboutContent(modelId, aboutContents[i], i + 1);
        }
...
exports.updateAdminModel = async (req, res, next) => {
    try {
        ...
        // Re-insert related data (same as postAdminModel)
        const exteriorColors = [];
        Object.keys(req.body).forEach(key => {
            if (key.startsWith('exteriorColorName')) {
                const index = key.match(/\d+$/)?.[0];
