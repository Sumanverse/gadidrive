...
const getModelById = async (req, res) => {
  ...
  try {
    ...
    const details = await Model.getModelDetails(modelId, conn); // MUST RETURN NESTED: exteriorColors[], exteriorColorImages[], specifications[], specificationLists[], specContents[]
...
const insertRelatedData = async (req, modelId, conn) => {
  ...
  for (const key of exteriorKeys) {
    ...
    const extraImgs = getFiles(files, `exteriorAdditionalColorImage${idx}_`);
  ...
  for (const key of interiorKeys) {
    ...
    const extraImgs = getFiles(files, `interiorAdditionalColorImage${idx}_`);
    for (const img of extraImgs) {
