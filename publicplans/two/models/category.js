...
// ---------- ADD THESE METHODS TO EXISTING Category CLASS ----------
...
static async getRelatedCategories(vehicleTypeId, currentCategoryId) {
    const [rows] = await db.execute(
        `SELECT c.*, v.vehicle_type_name
         FROM categories c
         JOIN vehicletype v ON c.vehicle_type_id = v.vehicle_type_id
         WHERE c.vehicle_type_id = ? AND c.category_id != ?
         ORDER BY c.name ASC
         LIMIT 4`,
        [vehicleTypeId, currentCategoryId]
    );
    return rows;
}
